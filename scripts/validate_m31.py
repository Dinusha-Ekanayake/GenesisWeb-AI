#!/usr/bin/env python3
"""
M31 Validation -- Full CRUD Frontend UI Foundation
===================================================
Validates that NextJsPlugin generates entity pages with edit and delete
functionality, completing the full CRUD loop against the generated FastAPI.

Sections:
  1.  Genesis backend health
  2.  Authentication
  3.  Generate frontend_full_crud_001 via propose+approve-and-generate
  4.  File tree: lib/api.ts, lib/types.ts, .env.example, entity pages
  5.  Content: api.ts exports (API_BASE_URL, all 5 CRUD functions)
  6.  Content: types.ts interfaces (Customer/Deal/Activity + rich fields)
  7.  Content: entity page full-CRUD checks (updateItem, deleteItem, Edit, Delete, editingId)
  8.  Frontend npm install
  9.  Frontend npm run build
  10. Generated backend py_compile
  11. Backward compat: simple_no_entities_001 (no lib/api.ts generated)

Usage:
    python scripts/validate_m31.py
    python scripts/validate_m31.py --backend-url http://127.0.0.1:8000
"""
...
FULL_CRUD_PROJECT_ID = "frontend_full_crud_001"
...
def _generate_project(api, auth, project_id, prompt, workspace_root):
    ...
    gen_payload = json.dumps({
        ...
        "approval": {
            "approved": True,
            "approved_by": "m31_validation",
            "notes": "M31 automated validation.",
        },
    }).encode()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M31 full CRUD frontend validation")
    ...
    project_ws = workspace_root / FULL_CRUD_PROJECT_ID
    frontend_dir = project_ws / "frontend"
    backend_dir = project_ws / "backend"

    print()
    print("M31 Validation -- Full CRUD Frontend UI Foundation")
    print("=" * 60)
    print(f"  Backend : {base}")
    print(f"  Project : {FULL_CRUD_PROJECT_ID}")
    print()
    ...
    # ------------------------------------------------------------------
    # 3. Generate frontend_full_crud_001
    # ------------------------------------------------------------------
    print(f"\n3. Generate {FULL_CRUD_PROJECT_ID}")
    print(f'  Prompt: "{CRM_PROMPT}"')

    plan = _generate_project(api, auth, FULL_CRUD_PROJECT_ID, CRM_PROMPT, workspace_root)
    if plan is None:
    ...
    if entity_pages_dir.exists():
        entity_page_files = [
            ...
            if d.is_dir() and (d / "page.tsx").exists() and d.name != "layout.tsx"
        ]
    ...
    # Spot-check expected CRM entity pages
    ...
    # ------------------------------------------------------------------
    # 5. api.ts content checks
    # ------------------------------------------------------------------
    print(f"\n5. api.ts Content")
    api_ts = frontend_dir / "lib" / "api.ts"
    if api_ts.exists():
        content = api_ts.read_text(encoding="utf-8")
        for symbol in ["API_BASE_URL", "listItems", "getItem", "createItem",
        ...
        if "8010" in content:
            ...
            _fail("api.ts default port is 8010", "8010 not found in api.ts")
        if "NEXT_PUBLIC_API_BASE_URL" in content:
            _pass("api.ts reads NEXT_PUBLIC_API_BASE_URL env var")
        else:
            _fail("api.ts reads NEXT_PUBLIC_API_BASE_URL env var", "not found")
    else:
        _fail("api.ts exists for content check", "MISSING")

    # ------------------------------------------------------------------
    # 6. types.ts content checks
    # ------------------------------------------------------------------
    print(f"\n6. types.ts Content")
    ...
    # ------------------------------------------------------------------
    # 7. Entity page full-CRUD content checks
    # ------------------------------------------------------------------
    print(f"\n7. Entity Page Full-CRUD Content")
    ...
    if customers_page.exists():
        ...
        # M30 baseline checks still apply
        ...
        # M31 new checks
        if "updateItem" in content:
            _pass("customers/page.tsx imports/calls updateItem")
        else:
            _fail("customers/page.tsx imports/calls updateItem", "not found")

        if "deleteItem" in content:
            _pass("customers/page.tsx imports/calls deleteItem")
        else:
            _fail("customers/page.tsx imports/calls deleteItem", "not found")

        if "editingId" in content:
            _pass("customers/page.tsx uses editingId state")
        else:
            _fail("customers/page.tsx uses editingId state", "not found")

        # M32 supersedes editingValue with form state — accept either approach
        if "editingValue" in content or "[form, setForm]" in content:
            _pass("customers/page.tsx tracks edit field state (editingValue or form)")
        else:
            _fail("customers/page.tsx tracks edit field state", "neither editingValue nor [form, setForm] found")

        if "Edit" in content:
            _pass("customers/page.tsx has Edit button")
        else:
            _fail("customers/page.tsx has Edit button", "not found")

        if "Delete" in content:
            _pass("customers/page.tsx has Delete button")
        else:
            _fail("customers/page.tsx has Delete button", "not found")

        if "Cancel" in content:
            _pass("customers/page.tsx has Cancel button")
        else:
            _fail("customers/page.tsx has Cancel button", "not found")

        if "handleDelete" in content:
            _pass("customers/page.tsx defines handleDelete")
        else:
            _fail("customers/page.tsx defines handleDelete", "not found")
    ...
    if not no_ent_api_ts.exists():
        ...
        _fail("simple_no_entities_001: lib/api.ts absent",
              "lib/api.ts generated for no-entity spec — should only appear for entity specs")
    ...
    if _failures:
        ...
        print("RESULT: PASS  (all M31 checks passed)")
        sys.exit(0)
